        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((loc) => (
            <Link
              ...
              className="group flex items-center justify-between rounded-xl border border-zinc-200 bg-white px-6 py-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
              <span className="font-medium text-zinc-900 group-hover:text-[var(--primary)]">
                {loc.city}
              </span>
              <ArrowRight className="h-4 w-4 text-zinc-400 transition group-hover:text-[var(--accent)] group-hover:translate-x-0.5" />
            </Link>
          ))}
        </div>
      </div>
